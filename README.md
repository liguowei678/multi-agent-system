...
基于 LangGraph 的 Supervisor 多智能体协同系统。Supervisor(temperature=0) 统一调度：意图识别 → 任务规划 → 动态分配子Agent → 循环决策 → 最终审查。集成 MCP 协议调用外部工具（27个），五层记忆检索（Redis → Qdrant → Neo4j → 关键词兜底 → SQLite），LLM 语义抽取写入知识图谱，支持跨会话记忆。
